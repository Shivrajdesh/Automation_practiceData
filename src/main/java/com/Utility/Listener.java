...
public class Listener extends BaseClass implements ITestListener { //right click on listener class then go to source click on orride/implement methods

	ExtentReports extent=ExtentReportGenerator.getReports();
	
	ThreadLocal<ExtentTest> extenttest=new ThreadLocal<ExtentTest>();//at one time it will write not for every test cases
